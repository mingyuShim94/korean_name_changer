...
export function generateKoreanNameSystemPrompt(
  ...
  const prompt: KoreanNamePromptStructure = {
    ...
    output_format: {
      structure: {
        ...
        korean_name_suggestion: {
          ...
          rationale:
            nameStyle === "pureKorean"
              ? "The surname 'Park' (박) was chosen to complement the natural, elemental feeling of the given name. 'Haneul' (하늘) means 'sky' in Korean, reflecting the celestial meaning in the original name. This preserves the essence of reaching upward and boundlessness while using a common Korean nature word that is modern and meaningful."
              : "The surname 'Loren' inspired the use of '김', one of the most culturally grounded Korean family names, resonating with nobility and tradition. The given name '서화' combines '서' (to express emotion) and '화' (elegance and beauty), symbolizing Sophia's depth of wisdom and poetic grace. Together, the two syllables form a name that suggests expressing inner beauty with sophistication—mirroring the emotional and symbolic essence of the original name.",
          life_values:
            nameStyle === "pureKorean"
              ? "A person who embodies the expansiveness of the sky, with a character marked by openness, visionary thinking, and the ability to embrace possibilities. They bring clarity and perspective to situations while remaining grounded in tradition."
              : "A person who expresses emotions with refinement and grace in speech and action. They cherish art, cultivate inner wisdom, and live a life of warm harmony with others.",
        },
        social_share_content: {
          formatted:
            nameStyle === "pureKorean"
    ...
    style_guidelines: [
      ...
      nameStyle === "pureKorean"
        ? "Choose Pure Korean words (순우리말) with beautiful, positive meanings reflecting the original name's essence."
      ...
      nameStyle === "pureKorean"
        ? "For Pure Korean names, explain the meaning of each syllable and its connection to Korean culture and natural elements."
...
// Example usage:
// const masculineHanjaPrompt = generateKoreanNameSystemPrompt({ gender: "masculine", nameStyle: "hanja" });
// const feminineKoreanPrompt = generateKoreanNameSystemPrompt({ gender: "feminine", nameStyle: "pureKorean" });
